...
class PositionalEncoding(nn.Module):
  def __init__(self,d_model:int,seq_len:int,dropout:float):
    ...
    #Create a matrix of d_model and seq_len
...
class MultiHeadAttentionBlock(nn.Module):
  def __init__(self,d_model:int,h:int,dropout:float):
    ...
    assert d_model%h ==0,"d model is not divisibel by h"
  ...
  def forward(self,q,k,v,mask):
    ...
    query = query.view(query.shape[1],self.h,self.d_k).transpose(1,2)
    key = key.view(key.shape[1],self.h,self.d_k).transpose(1,2)
    value = key.view(key.shape[1],self.h,self.d_k).transpose(1,2)
...
class EncoderBlock(nn.Module):
  def __init__(self, features:int,self_attention_block:MultiHeadAttentionBlock,feed_forward_block:FeedForwardBlock,dropout:float):
    ...
    self.self.feed_forward_block=feed_forward_block
